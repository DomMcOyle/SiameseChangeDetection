...
if __name__ == '__main__':
    ...
    if int(parser["settings"].get("training")) == 1:
        ...
        for i in range(0, len(first_img)):
            # preprocessing
            x_test, y_test = dp.preprocessing([first_img[i]], [second_img[i]], [labels[i]], parser[dataset_name], True)
            ...
            fig = s.plot_maps(prediction, dp.refactor_labels(np.asarray(labels[i]), parser[dataset_name]))
